...
export default class {
  constructor (options) {
    ...
    router.post('/settings/:key', this.setSetting.bind(this))
    router.get('/client-settings', this.getClientSettings.bind(this))
    router.post('/client-settings', this.setClientSettings.bind(this))
    router.get('/profile', this.getProfile.bind(this))
    router.get('/profile/:key', this.getProfileItem.bind(this))
    router.post('/profile/:key', this.setProfileItem.bind(this))
  ...
  getSelf (req, res, next) {
    this.models.User.findOne({ id: req.user.id }).select('-__v -_id -password')
      .then(user => {
        res.json(user)
      })
      .catch(err => next(err))
    ...
    bcrypt.hash(password, 10, (err, hash) => {
      ...
      this.models.User
        ...
        .then(user => {
          ...
          const roles = ['user']
          this.models.User.countDocuments({}).then(count => {
  ...
  getAllSettings (req, res, next) {
    this.models.User.findOne({ id: req.user.id })
      .then(user => {
  ...
  getClientSettings (req, res, next) {
    this.models.User.findOne({ id: req.user.id }).select('clientSettings')
      .then(user => {
  ...
  getUsername (req, res, next) {
    this.models.User.findOne({ id: req.user.id }).select('username')
      .then(user => {
  ...
  checkUsernameAvailability (req, res, next) {
    this.models.User.countDocuments({ username: req.body.username })
      .then(count => {
  ...
  setUsername (req, res, next) {
    this.models.User.countDocuments({ username: req.body.username })
      .then(count => {
        if (count !== 0) return res.status(406).json({ error: 'Username is unavailable' })
        this.models.User.findOneAndUpdate({ id: req.user.id }, { username: req.body.username })
          .then(user => {
